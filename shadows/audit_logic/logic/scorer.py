...
class AuditScorer:
    ...
    def score_repository(self, repo_scan) -> Dict[str, Any]:
        ...
        explanation = self._generate_explanation(
            gitops_score, security_score, architecture_score, 
            ci_cd_score, rbac_score, overall_score
        )
        
        recommendations = self._generate_recommendations(
            gitops_score, security_score, architecture_score,
            ci_cd_score, rbac_score
        )
    ...
    def _score_gitops_readiness(self, repo_scan) -> float:
        ...
        if any(tool.get("name") in ["argocd", "flux", "tekton"] for tool in gitops_tools):
            score += 30.0
    ...
    def _score_security_signals(self, repo_scan) -> float:
        ...
        if not secrets_mgmt:
            score -= 20.0
        elif any("plaintext" in secret.get("type", "").lower() for secret in secrets_mgmt):
            score -= 15.0
    ...
    def _score_architecture_health(self, repo_scan) -> float:
        ...
        if any("microservice" in pattern.get("type", "").lower() for pattern in arch_patterns):
            score += 40.0
        elif any("monolith" in pattern.get("type", "").lower() for pattern in arch_patterns):
            score += 20.0
            
        # Check for service discovery
        if any("service-mesh" in pattern.get("type", "").lower() for pattern in arch_patterns):
            score += 20.0
            
        # Check for API gateway
        if any("api-gateway" in pattern.get("type", "").lower() for pattern in arch_patterns):
            score += 20.0
            
        # Check for health checks
        if any("health-check" in pattern.get("type", "").lower() for pattern in arch_patterns):
            score += 20.0
    ...
    def _score_rbac_compliance(self, repo_scan) -> float:
        ...
        if rbac_configs:
            ...
            if any("serviceaccount" in config.get("content", "").lower() for config in rbac_configs):
                score += 20.0
    ...
    def _generate_explanation(self, gitops_score: float, security_score: float, 
                            architecture_score: float, ci_cd_score: float, 
                            rbac_score: float, overall_score: float) -> str:
        """Generate explanation for the audit score"""
        explanations = []
        
        if overall_score >= 0.8:
            explanations.append("Excellent repository health with strong adherence to best practices.")
        elif overall_score >= 0.6:
            explanations.append("Good repository health with room for improvement in specific areas.")
        elif overall_score >= 0.4:
            explanations.append("Moderate repository health with several areas needing attention.")
        else:
            explanations.append("Poor repository health requiring immediate attention and remediation.")
            
        # Add specific area explanations
        if gitops_score < 0.5:
            explanations.append("GitOps practices need improvement - consider implementing ArgoCD or Flux.")
        if security_score < 0.6:
            explanations.append("Security posture requires enhancement - review secrets management and access controls.")
        if architecture_score < 0.5:
            explanations.append("Architecture could benefit from more modular design patterns.")
            
        return " ".join(explanations)

    def _generate_recommendations(self, gitops_score: float, security_score: float,
                                architecture_score: float, ci_cd_score: float,
                                rbac_score: float) -> List[str]:
