...
class AgentLauncher:
    ...
    def launch_shadow_agent(self, logic_source: str, deployment_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not success:
                return {
                    'success': False,
                    'error': 'Failed to apply ArgoCD application'
                }
                
            # Update registry
            self.registry.update_agent_config(logic_source, {
                'deployment_status': 'deployed',
                'last_deployed': datetime.now().isoformat()
            })
            
            # Record deployment
            self.registry.record_deployment(logic_source, {
                'action': 'deploy',
    ...
    def _create_argocd_app_manifest(self, agent_name: str, helm_chart: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """Create ArgoCD application manifest"""
    ...
    def _config_to_helm_values(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        helm_values = {
            'replicaCount': 1,
            'image': {
                'repository': config.get('image', 'ghcr.io/shadow-link-industries/placeholder'),
                'tag': 'latest',
                'pullPolicy': 'Always'
            },
            'service': {
    ...
    def _apply_argocd_application(self, app_manifest: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(f"Successfully applied ArgoCD application: {app_manifest['metadata']['name']}")
                return True
            else:
            ...
            for app in apps_data.get('items', []):
                ...
                if app_name.endswith('-logic'):
                    ...
                    launched_agents.append({
                        'agent_name': agent_name,
                        'app_name': app_name,
                        'status': app['status'].get('health', {}).get('status', 'Unknown'),
    ...
    def get_agent_status(self, agent_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run([
                'kubectl', 'get', 'application', app_name, '-n', self.argocd_namespace,
                '-o', 'json'
            ], capture_output=True, text=True)
            ...
            deploy_result = subprocess.run([
                'kubectl', 'get', 'deployment', deployment_name, '-n', self.linkops_namespace,
                '-o', 'json'
            ], capture_output=True, text=True)
            ...
            if deploy_result.returncode == 0:
                ...
                deployment_status = {
                    ...
                    'available_replicas': deploy_data['status'].get('availableReplicas', 0),
            ...
            return {
                ...
                'app_name': app_name,
                'argocd_status': {
                    'health': app_data['status'].get('health', {}).get('status', 'Unknown'),
                    'sync_status': app_data['status'].get('sync', {}).get('status', 'Unknown'),
                    'last_sync': app_data['status'].get('operationState', {}).get('finishedAt', 'Unknown')
                },
