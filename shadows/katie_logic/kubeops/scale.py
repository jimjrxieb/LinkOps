...
class KubernetesScaler:
    ...
    def scale_deployment(
        ...
        try:
            logger.info(
                f"Katie scaling deployment {deployment_name} to {replicas} replicas"
            )
    ...
    def scale_statefulset(
        ...
        try:
            logger.info(
                f"Katie scaling statefulset {statefulset_name} to {replicas} replicas"
            )

            if dry_run:
                return self._dry_run_scale_statefulset(
                    namespace, statefulset_name, replicas
                )
    ...
    def scale_from_zero(
        self, namespace: str, deployment_name: str, replicas: int = 1
    ) -> Dict[str, Any]:
        """
    ...
    def get_scaling_recommendations(
        ...
        try:
            logger.info(
                f"Katie analyzing scaling recommendations for {deployment_name}"
            )
            ...
            # Get resource usage
            resource_usage = self._get_deployment_resource_usage(
    ...
    def _dry_run_scale_statefulset(
    ...
    def _analyze_scaling_impact(
        self, namespace: str, deployment_name: str, current: int, target: int
    ) -> Dict[str, Any]:
        """Analyze the impact of scaling operation"""
        impact = {
            "replica_change": target - current,
            "percentage_change": (
                ((target - current) / current * 100) if current > 0 else 0
            ),
    ...
    def _analyze_statefulset_scaling_impact(
        self, namespace: str, statefulset_name: str, current: int, target: int
    ) -> Dict[str, Any]:
        """Analyze the impact of statefulset scaling operation"""
        impact = self._analyze_scaling_impact(
            namespace, statefulset_name, current, target
        )
...
"""
    ...
    def _get_deployment_resource_usage(
        self, namespace: str, deployment_name: str
    ) -> Dict[str, Any]:
    ...
    def _generate_scaling_recommendations(
        ...
        if (
            current_replicas > 5
            and resource_usage.get("cpu_usage") == "estimated_60_percent"
        ):
            recommendations.append(
                {
                    "type": "scale_down",
    ...
    def _generate_scaling_insight(
        self, deployment_name: str, current: int, target: int, impact: Dict[str, Any]
    ) -> str:
    ...
    def _generate_statefulset_scaling_insight(
        ...
        base_insight = self._generate_scaling_insight(
            statefulset_name, current, target, impact
        )
