...
class AgentLogScraper:
    ...
    def __init__(self, logs_base_path: str = "/app/logs"):
        ...
        self.agent_patterns = {
            ...
            'igris': {
                ...
                'keywords': ['infrastructure', 'security', 'analysis', 'recommendation'],
    ...
    def _scrape_single_agent_logs(self, agent_name: str, config: Dict[str, Any], cutoff_time: datetime) -> List[Dict[str, Any]]:
        ...
        if not log_files:
            logger.warning(f"No log files found for {agent_name} with pattern {log_pattern}")
            return log_entries
    ...
    def _parse_log_file(self, log_file: str, agent_name: str, config: Dict[str, Any], cutoff_time: datetime) -> List[Dict[str, Any]]:
                ...
                for line_num, line in enumerate(f, 1):
                    try:
                        ...
                        if parsed_entry:
                            ...
                            if parsed_entry.get('timestamp') and parsed_entry['timestamp'] >= cutoff_time:
                                # Check if entry contains relevant keywords
                                if self._is_relevant_entry(parsed_entry.get('message', ''), config['keywords']):
                        ...
                        logger.debug(f"Error parsing line {line_num} in {log_file}: {str(e)}")
                        continue
    ...
    def extract_intelligence_patterns(self, log_entries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for entry in log_entries:
            ...
            # Success patterns
            elif level == 'INFO' and any(word in message for word in ['success', 'completed', 'finished']):
            ...
            elif any(word in message for word in ['performance', 'latency', 'throughput', 'cpu', 'memory']):
            ...
            # Usage patterns
            elif any(word in message for word in ['request', 'api', 'endpoint', 'call']):
            ...
            # Recommendations
            elif any(word in message for word in ['recommend', 'suggest', 'advice', 'best practice']):
    ...
    def _generate_insights(self, patterns: Dict[str, Any]) -> List[str]:
        ...
        if error_count > 0:
            insights.append(f"Found {error_count} error patterns that may need attention")
        ...
        if perf_count > 0:
            insights.append(f"Detected {perf_count} performance-related patterns for optimization")
            
        # Usage insights
        usage_count = len(patterns['usage_patterns'])
        if usage_count > 0:
            insights.append(f"Tracked {usage_count} usage patterns for capacity planning")
    ...
    def _generate_recommendations(self, patterns: Dict[str, Any]) -> List[str]:
        ...
        if patterns['performance_patterns']:
            recommendations.append("Analyze performance patterns for optimization opportunities")
            
        # Usage-based recommendations
        if patterns['usage_patterns']:
            recommendations.append("Monitor usage patterns for capacity planning and scaling")
            
        # Success-based recommendations
        if patterns['success_patterns']:
            recommendations.append("Document successful operation patterns for best practices")
