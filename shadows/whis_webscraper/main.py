...
async def scrape_intelligence(
    ...
    try:
        ...
        if "agent_logs" in request.sources:
            ...
            all_scraped_data["agent_logs"] = log_scraper.scrape_agent_logs(
                request.hours_back
            )
...
async def scrape_agent_logs(hours_back: int = 24, send_to_sanitize: bool = True):
    ...
    try:
        ...
        if send_to_sanitize and log_entries:
            ...
            log_data = {"agent_logs": log_entries, "intelligence_patterns": patterns}
        ...
        raise HTTPException(
            status_code=500, detail=f"Agent log scraping failed: {str(e)}"
        )
...
async def scrape_github_trending(send_to_sanitize: bool = True):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"GitHub trending scraping failed: {str(e)}"
        )
...
async def get_sanitize_stats():
    ...
    try:
        ...
        return {"sanitize_stats": stats, "timestamp": datetime.now().isoformat()}
    except Exception as e:
        return {
...
def get_capabilities():
    ...
    return {
        ...
        "supported_sources": list(web_scraper.dev_sources.keys())
        + list(web_scraper.github_trending.keys()),
