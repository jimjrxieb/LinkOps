...
from db.models import Orb, Rune, Log, WhisQueue, Approval, Task
...
async def create_task(task: TaskInput, db: Session = Depends(get_db)):
    ...
    # Simulated agent confidence scores (basic logic)
    scores = {
        "katie": 90 if "kubernetes" in input_text or "pod" in input_text else 10,
        "igris": 90 if "terraform" in input_text or "vault" in input_text else 10,
        "whis": 90 if "train" in input_text or "ml" in input_text else 10
    }
    task = Task(
        input_text=task.input_text,
        agent_rankings=json.dumps(scores)
    )
    db.add(task)
    db.commit()
    db.refresh(task)
    return {"id": task.id, "rankings": scores}
