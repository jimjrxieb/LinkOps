...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = Field(
        default="postgresql://linkops:linkops@postgres:5432/linkops", env="DATABASE_URL"
    )
    ...
    # Service URLs
    WHIS_SANITIZE_URL: str = Field(
        default="http://whis_sanitize:8000", env="WHIS_SANITIZE_URL"
    )
    WHIS_SMITHING_URL: str = Field(
        default="http://whis_smithing:8000", env="WHIS_SMITHING_URL"
    )
    WHIS_ENHANCE_URL: str = Field(
        default="http://whis_enhance:8000", env="WHIS_ENHANCE_URL"
    )
...
def validate_required_settings():
    ...
    required_settings = ["DATABASE_URL", "KAFKA_BROKERS", "SECRET_KEY"]
