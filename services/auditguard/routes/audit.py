    ...
    try:
        ...
        for scope in request.audit_scope:
            if scope == "secrets":
                ...
                findings["credentials"] = await _check_for_credentials(request.repository_path)
            elif scope == "sensitive_files":
                findings["sensitive_files"] = await _check_for_sensitive_files(request.repository_path)
...
def _calculate_risk_score(findings: Dict[str, Any]) -> float:
    ...
    if findings.get("credentials"):
        risk_score += len(findings["credentials"]) * 0.2  # Medium weight for credentials
    
    if findings.get("sensitive_files"):
        risk_score += len(findings["sensitive_files"]) * 0.25  # High weight for sensitive files
...
def _generate_recommendations(findings: Dict[str, Any]) -> List[str]:
    ...
    if findings.get("credentials"):
        recommendations.append("Remove hardcoded credentials and use secure authentication")
