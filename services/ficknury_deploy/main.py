...
async def deploy_agent(request: DeployRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not request.force_redeploy:
            deployment_history = agent_registry.get_deployment_history(request.logic_source)
        ...
        logger.info(f"Successfully deployed {agent_config['agent_name']} from {request.logic_source}")
        return response
...
async def undeploy_agent(agent_name: str):
    ...
    try:
        ...
        agent_registry.update_agent_config(logic_source, {
...
async def list_registered_agents():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list registry: {str(e)}")
...
async def register_agent(logic_source: str, agent_config: Dict[str, Any]):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to register agent {logic_source}"
            )
...
async def update_agent_config(logic_source: str, updates: Dict[str, Any]):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"Agent {logic_source} not found in registry"
            )
