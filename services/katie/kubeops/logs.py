...
class KubernetesLogAnalyzer:
    ...
    def search_logs(
        ...
        try:
            ...
            # Get pods to search
            if pod_selector:
                pods = self.v1.list_namespaced_pod(namespace, label_selector=pod_selector)
            else:
    ...
    def get_log_summary(
        ...
        """
        try:
            logger.info(f"Katie generating log summary for {deployment_name or namespace}")
            ...
            for pod in pods.items:
                ...
                summary_data[pod_name] = {
                    ...
                    "error_count": len([l for l in log_lines if 'error' in l.lower()]),
                    "warning_count": len([l for l in log_lines if 'warn' in l.lower()]),
    ...
    def _analyze_logs(self, logs: str) -> Dict[str, Any]:
        ...
        if not logs:
            return {"error_count": 0, "warning_count": 0, "info_count": 0, "severity": "normal"}
    ...
    def _get_most_common_errors(self, error_patterns: Dict[str, Dict[str, Any]]) -> List[str]:
    ...
    def _generate_log_insight(self, analysis: Dict[str, Any], key_events: List[Dict[str, Any]]) -> str:
        ...
        if analysis["severity"] == "critical":
            ...
            return f"Moderate log activity: {analysis['warning_count']} warnings detected."
        else:
            return f"Normal log activity: {analysis['total_lines']} log entries analyzed."

    def _generate_deployment_log_insight(self, deployment_name: str, pod_count: int, analysis: Dict[str, Any]) -> str:
    ...
    def _generate_search_insight(self, pattern: str, match_count: int, pod_count: int) -> str:
    ...
    def _generate_summary_insight(self, target: str, total_logs: int, pod_count: int) -> str:
