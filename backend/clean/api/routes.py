...
async def log_action(
    ...
    action: str = Body(...),
    result: str = Body(...),
    solution_path: Optional[str] = Body(None),
    error_outcome: Optional[str] = Body(None),
    sanitized: bool = Body(False),
    approved: bool = Body(False),
    auto_approved: bool = Body(False),
    compliance_tags: Optional[List[str]] = Body(None),
    db: Session = Depends(get_db)
):
    """Log agent action with PwC-aligned fields"""
    try:
        log_data = {
            "agent": agent,
            "task_id": task_id,
        ...
        return {
            ...
            "message": f"PwC-aligned log created for {agent}"
        }
...
async def get_agent_capabilities(db: Session = Depends(get_db)):
    """Get status of all agents - updated with new agents"""
    return {
        "katie": "Kubernetes Pro (CKA/CKS)",
        "igris": "Platform + DevSecOps", 
        "james": "Jarvis-like task router",
        "whis": "ML Engine active",
        "auditguard": "Security & Compliance Specialist",
        "ficknury": "Agent Orchestration & Deployment"
    }
...
@router.post("/api/auditguard/scan")
async def auditguard_scan(
    scan_type: str = Body(...),
    target: str = Body(...),
    compliance_scope: List[str] = Body(["SOC2", "GDPR"]),
    auto_approve: bool = Body(False),
    db: Session = Depends(get_db)
):
    """Execute AuditGuard security scan"""
    try:
        ...
        # Execute scan (this would call AuditGuard agent)
        # For now, simulate the scan
        scan_result = {
            "agent": "auditguard",
            "task_id": task_id,
            "action": f"{scan_type} scan complete",
            "result": {
                "high_risk": 2 if scan_type == "trivy" else 0,
                "medium_risk": 5 if scan_type == "bandit" else 0,
                "low_risk": 10 if scan_type == "checkov" else 0,
                "notes": "[SECRET_DETECTED] Placeholder inserted" if scan_type == "repo_audit" else "Scan completed successfully",
                "compliance_tags": compliance_scope
            },
            "solution_path": f"Update {scan_type} findings and implement security controls",
            "error_outcome": None,
            "sanitized": True,
            "approved": auto_approve,
            "auto_approved": auto_approve,
            "compliance_tags": json.dumps(compliance_scope)
        }
        
        # Log the audit result
        log_entry = Log(**scan_result)
        db.add(log_entry)
        db.commit()
        ...
        return {
            ...
            "scan_type": scan_type,
            "result": scan_result,
            "message": f"AuditGuard {scan_type} scan completed successfully"
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"AuditGuard scan failed: {str(e)}")

@router.post("/api/ficknury/propose-agent")
async def ficknury_propose_agent(
    agent_name: str = Body(...),
    agent_type: str = Body(...),
    intelligence_source: str = Body(...),
    reasoning: str = Body(...),
    capabilities: List[str] = Body(...),
    deployment_target: str = Body(...),
    priority: str = Body("medium"),
    db: Session = Depends(get_db)
):
    """Propose agent creation/upgrade/deployment via FickNury"""
    try:
        task_id = str(uuid.uuid4())
        ...
        # Analyze proposal (this would call FickNury agent)
        ...
        decision = {
            "approved": True,
            "reason": "High feasibility and impact",
            "auto_approved": True,
            "solution_path": f"Deploy {agent_name} to {deployment_target}"
        }
        ...
        orchestration_result = {
            "agent": "ficknury",
            "task_id": task_id,
            "action": f"Agent {agent_type} proposal processed",
            "result": {
                "proposal": proposal,
                "analysis": analysis,
                "decision": decision,
                "deployment_result": {"status": "pending_deployment"}
            },
            "solution_path": decision["solution_path"],
            "error_outcome": None,
            "sanitized": True,
            "approved": decision["approved"],
            "auto_approved": decision["auto_approved"],
            "compliance_tags": json.dumps(["ISO27001", "NIST"])
        }
        
        log_entry = Log(**orchestration_result)
        db.add(log_entry)
        db.commit()
        ...
        return {
            ...
            "analysis": analysis,
            "decision": decision,
            "message": f"FickNury processed {agent_type} proposal for {agent_name}"
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"FickNury proposal failed: {str(e)}")

@router.post("/api/ficknury/deploy-agent")
async def ficknury_deploy_agent(
    agent_name: str = Body(...),
    version: str = Body(...),
    deployment_target: str = Body(...),
    ...
    try:
        ...
        # Execute deployment (this would call FickNury agent)
        ...
        deployment_result = {
            ...
            "agent_name": agent_name,
            "version": version,
            "solution_path": f"Agent {agent_name} version {version} deployed to {deployment_target}"
        }
        ...
        deployment_log = {
            "agent": "ficknury",
            "task_id": task_id,
            "action": f"Agent deployment to {deployment_target}",
            "result": json.dumps(deployment_result),
            "solution_path": deployment_result["solution_path"],
            "error_outcome": None,
            "sanitized": True,
            "approved": auto_approve,
            "auto_approved": auto_approve,
        ...
        return {
            ...
            "message": f"FickNury deployed {agent_name} version {version} to {deployment_target}"
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"FickNury deployment failed: {str(e)}")
...
async def get_audit_logs(
    ...
    """Get PwC-aligned audit logs with filtering"""
    try:
        query = db.query(Log)
        
        # Apply filters
        if agent:
            query = query.filter(Log.agent == agent)
        
        if compliance_tag:
            query = query.filter(Log.compliance_tags.contains(compliance_tag))
        
        if approved_only:
            query = query.filter(
                (Log.approved == True) | (Log.auto_approved == True)
            )
...
async def get_compliance_stats(db: Session = Depends(get_db)):
    """Get compliance statistics across all agents"""
    try:
        ...
        for log in approved_logs:
            # Agent statistics
            agent_stats[log.agent] = agent_stats.get(log.agent, 0) + 1
            
            # Compliance tag statistics
            if log.compliance_tags:
                try:
                    tags = json.loads(log.compliance_tags)
                    for tag in tags:
                        compliance_stats[tag] = compliance_stats.get(tag, 0) + 1
                except json.JSONDecodeError:
                    continue
        
        return {
            "total_approved_logs": len(approved_logs),
            "compliance_tag_breakdown": compliance_stats,
            "agent_breakdown": agent_stats,
            "top_compliance_tags": sorted(compliance_stats.items(), key=lambda x: x[1], reverse=True)[:5]
        }
